...
function Player () {
    ...
    const fetchCurrentSong = () => {
        if (!songInfo) {
           spotifyApi.getMyCurrentPlayingTrack().then((data) => {
               console.log("Ahora esta sonando: ", data.body?.item);
    ...
    useEffect(() =>{
        if (spotifyApi.getAccessToken() && !currentTrackId){
            // fetch la informacion de la cancion
            fetchCurrentSong();
            setVolume(50);
        }
    ...
    const debouncedAdjustVolume = useCallback (
        debounce((volume) => {
            spotifyApi.setVolume(volume).catch((err) => {
                console.log(`No esta sonando`);
            });
        }, 500),
        []
    )
    console.log(`Esta reproduciendo ${songInfo}`);
    return (
        <div className="h-24 bg-gradient-to-b from-black to-gray-900
            ...
            <div className="flex items-center justify-evenly">
                ...
                <RewindIcon
                    // onClick={() => spotifyApi.skipToPrevius()} -- El API no esta funcionando
                    className="button"
                />
